...
class DocumentValidator:
    ...
    def validate_content(content: str, file_path: str = None) -> ValidationResult:
        ...
        if not isinstance(content, str):
            result.add_warning(
                "content", f"Content is not string type: {type(content)}"
            )
            content = str(content)
        ...
        if len(content) < 10:
            result.add_warning(
                "content", f"Content is very short ({len(content)} chars)"
            )
        elif len(content) > 1000000:  # 1MB limit
            result.add_warning(
                "content", f"Content is very large ({len(content)} chars)"
            )
        ...
        sanitized_file_path = (
            ContentSanitizer.sanitize_file_path(file_path)
            if file_path
            else "unknown_source"
        )
    ...
    def validate_chunk(chunk_data: Dict[str, Any]) -> ValidationResult:
        ...
        if "content" in chunk_data:
            content_result = DocumentValidator.validate_content(
                chunk_data["content"], chunk_data.get("file_path")
            )
    ...
    def validate_entity(entity_data: Dict[str, Any]) -> ValidationResult:
        ...
        if "entity_name" in entity_data:
            ...
            if not name or not str(name).strip():
                ...
                result.add_warning(
                    "entity_name", f"Entity name is very long ({len(str(name))} chars)"
                )
        ...
        if not result.has_errors():
            ...
            if "entity_name" in entity_data:
                sanitized_data["entity_name"] = ContentSanitizer.sanitize_entity_name(
                    str(entity_data["entity_name"])
                )

            if "entity_type" in entity_data:
                sanitized_data["entity_type"] = ContentSanitizer.sanitize_text(
                    str(entity_data["entity_type"]), 100
                )

            if "description" in entity_data:
                sanitized_data["description"] = ContentSanitizer.sanitize_text(
                    str(entity_data["description"])
                )
            ...
            for field in ["source_id", "file_path"]:
                if field in entity_data:
                    if field == "file_path":
                        sanitized_data[field] = ContentSanitizer.sanitize_file_path(
                            str(entity_data[field])
                        )
                    else:
                        sanitized_data[field] = ContentSanitizer.sanitize_text(
                            str(entity_data[field])
                        )
...
class RelationshipValidator:
        ...
        if "weight" in relationship_data:
            ...
            try:
                ...
                if weight_float < 0:
                    ...
                    result.add_warning(
                        "weight", f"Very high weight detected: {weight_float}"
                    )
        ...
        if not result.has_errors():
            ...
            for field in ["src_id", "tgt_id"]:
                if field in relationship_data:
                    sanitized_data[field] = ContentSanitizer.sanitize_entity_name(
                        str(relationship_data[field])
                    )

            if "description" in relationship_data:
                sanitized_data["description"] = ContentSanitizer.sanitize_text(
                    str(relationship_data["description"])
                )

            if "keywords" in relationship_data:
                sanitized_data["keywords"] = ContentSanitizer.sanitize_text(
                    str(relationship_data["keywords"])
                )
            ...
            for field in type_fields:
                if field in relationship_data:
                    sanitized_data[field] = (
                        str(relationship_data[field])
                        if relationship_data[field] is not None
                        else None
                    )
    ...
    def validate_relationship_batch(
        ...
        for i, relationship in enumerate(relationships):
            validation_result = RelationshipValidator.validate_relationship(
                relationship
            )
    ...
    def validate_node_data(node_data: Dict[str, Any]) -> ValidationResult:
        ...
        if "entity_id" in node_data:
            ...
            if not entity_id or not str(entity_id).strip():
                ...
                result.add_error(
                    "entity_id", f"Entity ID is too long ({len(str(entity_id))} chars)"
                )
        ...
        for field in string_fields:
            if field in node_data and node_data[field] is not None:
                if len(str(node_data[field])) > 50000:  # Reasonable limit for Neo4j
                    result.add_warning(
                        field,
                        f"Field '{field}' is very large ({len(str(node_data[field]))} chars)",
                    )
    ...
    def validate_edge_data(edge_data: Dict[str, Any]) -> ValidationResult:
        ...
        if "weight" in edge_data:
            try:
                ...
                result.add_error(
                    "weight", f"Invalid weight value: {edge_data['weight']}"
                )
...
def validate_extraction_results(
    ...
    valid_relationships, relationship_errors = (
        RelationshipValidator.validate_relationship_batch(relationships)
    )
    ...
    if all_errors:
        ...
        logger.debug(
            f"Valid relationships: {len(valid_relationships)}/{len(relationships)}"
        )
    else:
        logger.debug(
            f"Validation successful: {len(valid_entities)} entities, {len(valid_relationships)} relationships"
        )
...
def log_validation_errors(errors: List[ValidationError], context: str = ""):
    ...
    if error_count > 0:
        logger.error(
            f"{context_prefix}Validation found {error_count} errors, {warning_count} warnings"
        )
    ...
    if len(errors) > 10:
        logger.info(
            f"{context_prefix}... and {len(errors) - 10} more validation issues"
        )
