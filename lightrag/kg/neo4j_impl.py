...
from ..validation import DatabaseValidator, log_validation_errors  # Add validation import
from .utils.semantic_utils import process_relationship_weight, calculate_semantic_weight, get_default_threshold_manager, set_default_threshold_manager
from .utils.threshold_manager import ThresholdManager
from .utils.relationship_registry import RelationshipTypeRegistry, standardize_relationship_type
...
class ConnectionHealthMonitor:
    ...
    def record_failure(self):
        ...
        if self.failure_count >= self.max_failures and not self.is_circuit_open:
            ...
            utils.logger.error(f"Circuit breaker opened after {self.failure_count} failures in {self.failure_window}s")
    ...
    def should_allow_request(self) -> bool:
        ...
        if current_time - self.circuit_open_time > self.circuit_timeout:
            utils.logger.info("Circuit breaker attempting to close - allowing test request")
            return True
...
class Neo4JStorage(BaseGraphStorage):
    def __init__(
        ...
        **kwargs
        ...
        self.ssl = ssl or os.getenv("NEO4J_SSL", "false").lower() in ["true", "1", "yes"]
        ...
        self.enable_graceful_degradation = kwargs.get("enable_graceful_degradation", True)
    ...
    async def initialize(self):
        ...
        self.pg_connection_params = {
            "host": os.environ.get("POSTGRES_HOST", config.get("postgres", "host", fallback="localhost")),
            "port": int(os.environ.get("POSTGRES_PORT", config.get("postgres", "port", fallback="5432"))),
            "user": os.environ.get("POSTGRES_USER", config.get("postgres", "user", fallback="postgres")),
            "password": os.environ.get("POSTGRES_PASSWORD", config.get("postgres", "password", fallback="")),
            "database": os.environ.get("POSTGRES_DATABASE", config.get("postgres", "database", fallback="postgres")),
        }
                ...
                if e.code == "Neo.ClientError.Database.DatabaseNotFound":
                    ...
                    try:
                        async with self._driver.session() as session:
                            ...
                            await result.consume()  # Ensure result is consumed
                            utils.logger.info(f"{database} at {URI} created".capitalize())
                            connected = True
                    except (
        ...
        utils.logger.info(f"Neo4j storage initialized with relationship registry containing {len(self.rel_registry.registry)} types")
    ...
    async def _check_connection_health(self) -> bool:
        ...
        try:
            ...
            async with asyncio.wait_for(
                self._driver.session(database=self._DATABASE).__aenter__(), 
                timeout=5.0
            ) as session:
    ...
    async def _ensure_connection(self) -> bool:
        ...
        try:
            ...
            MAX_CONNECTION_POOL_SIZE = int(os.environ.get("NEO4J_MAX_CONNECTION_POOL_SIZE", 
                config.get("neo4j", "connection_pool_size", fallback=50)))
            CONNECTION_TIMEOUT = float(os.environ.get("NEO4J_CONNECTION_TIMEOUT",
                config.get("neo4j", "connection_timeout", fallback=30.0)))
            CONNECTION_ACQUISITION_TIMEOUT = float(os.environ.get("NEO4J_CONNECTION_ACQUISITION_TIMEOUT",
                config.get("neo4j", "connection_acquisition_timeout", fallback=30.0)))
            MAX_TRANSACTION_RETRY_TIME = float(os.environ.get("NEO4J_MAX_TRANSACTION_RETRY_TIME",
                config.get("neo4j", "max_transaction_retry_time", fallback=30.0)))
    ...
    async def _execute_with_retry(self, operation_func, *args, **kwargs):
            ...
            try:
                ...
                if not await self._ensure_connection():
                    if self.enable_graceful_degradation:
                        utils.logger.warning("Database unavailable, but graceful degradation is enabled")
                        return None
                    else:
                ...
                last_exception = e
                utils.logger.warning(f"Database operation timed out (attempt {retry_count + 1}/{max_retries})")
                self.health_monitor.record_failure()
                
            except (neo4jExceptions.ServiceUnavailable, 
                    neo4jExceptions.SessionExpired,
                    neo4jExceptions.TransientError) as e:
                last_exception = e
                utils.logger.warning(f"Transient database error (attempt {retry_count + 1}/{max_retries}): {str(e)}")
                self.health_monitor.record_failure()
                self._is_connected = False
                
            except Exception as e:
                last_exception = e
                utils.logger.error(f"Database operation failed (attempt {retry_count + 1}/{max_retries}): {str(e)}")
                self.health_monitor.record_failure()
                
                # Don't retry for certain types of errors
                if isinstance(e, (neo4jExceptions.AuthError, neo4jExceptions.ClientError)):
                    break
        ...
        # All retries failed
        if self.enable_graceful_degradation:
            utils.logger.warning(f"Database operation failed after {max_retries} retries, graceful degradation enabled")
            return None
        else:
    ...
    async def get_entity_embedding(self, entity_id: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            utils.logger.debug(f"No embedding found for entity {entity_id} in workspace {self.namespace}")
            return None
            
        except Exception as e:
            utils.logger.warning(f"Error retrieving embedding for {entity_id}: {str(e)}")
            return None
    
    async def batch_get_entity_embeddings(self, entity_ids: List[str]) -> Dict[str, np.ndarray]:
        """
        ...
        try:
            ...
            if to_fetch:
                utils.logger.debug(f"Could not find embeddings for {len(to_fetch)} entities in workspace {self.namespace}")
    ...
    async def has_node(self, node_id: str) -> bool:
        ...
        async with self._driver.session(
            ...
            try:
                ...
                utils.logger.debug(f"Executing Cypher query in has_node (exact): {query} with params: {{'entity_id': '{node_id}'}}")
                ...
                """
                utils.logger.debug(f"Executing Cypher query in has_node (flexible): {flexible_query} with params: {{'term': '{node_id}'}}")
                ...
                utils.logger.error(f"Error checking node existence for {node_id}: {str(e)}")
    ...
    async def has_edge(self, source_node_id: str, target_node_id: str) -> bool:
        ...
        async with self._driver.session(
            ...
            try:
                ...
                utils.logger.debug(f"Executing Cypher query in has_edge: {query} with params: {{'source_entity_id': '{source_node_id}', 'target_entity_id': '{target_node_id}'}}")
                result = await session.run(
                    query,
    ...
    async def get_node(self, node_id: str) -> dict[str, str] | None:
        ...
        async with self._driver.session(
            ...
            try:
                ...
                utils.logger.debug(f"Executing Cypher query in get_node (exact): {query} with params: {{'entity_id': '{node_id}'}}")
                result = await session.run(query, entity_id=node_id)
                ...
                RETURN n LIMIT 1
                """
                utils.logger.debug(f"Executing Cypher query in get_node (flexible): {flexible_query} with params: {{'term': '{node_id}'}}")
                result = await session.run(flexible_query, term=node_id)
                records = await result.fetch(2)
                await result.consume()
                ...
                if records:
                    ...
                    if "labels" in node_dict:
                        ...
                        # Ensure labels field always exists, using entity_type as fallback if available
                        node_dict["labels"] = [node_dict.get("entity_type", "unknown")]
                    utils.logger.info(f"Found node via flexible match for term: {node_id}")
                    return node_dict
    ...
    async def get_nodes_batch(self, node_ids: list[str]) -> dict[str, dict]:
        ...
        async with self._driver.session(
            ...
            """
            utils.logger.debug(f"Executing Cypher query in get_nodes_batch: {query} with params: {{'node_ids': {node_ids}}}")
    ...
    async def node_degree(self, node_id: str) -> int:
        ...
        async with self._driver.session(
            ...
            try:
                ...
                """
                utils.logger.debug(f"Executing Cypher query in node_degree: {query} with params: {{'entity_id': '{node_id}'}}")
                result = await session.run(query, entity_id=node_id)
                try:
                    record = await result.single()
    ...
    async def node_degrees_batch(self, node_ids: list[str]) -> dict[str, int]:
        ...
        async with self._driver.session(
            ...
            try:
                ...
                """
                utils.logger.debug(f"Executing Cypher query in node_degrees_batch: {query} with params: {{'node_ids': {node_ids}}}")
                result = await session.run(query, node_ids=node_ids)
                ...
                for nid in node_ids:
                    try:
                        ...
                        utils.logger.error(f"Failed to get degree for node '{nid}': {str(inner_e)}")
                        degrees[nid] = 0
                return degrees
    ...
    async def get_edge(
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                utils.logger.debug(f"Executing Cypher query in get_edge: {query} with params: {{'source_id': '{source_id}', 'target_id': '{target_id}', 'rel_type': '{rel_type}'}}")
                result = await session.run(
                    query,
                    source_id=source_id,
                    target_id=target_id,
    ...
    async def get_edges_batch(
        ...
        for i, pair in enumerate(pairs):
            if "src" not in pair or "tgt" not in pair:
                utils.logger.warning(f"Invalid edge pair: {pair}, missing 'src' or 'tgt' key")
                continue
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                utils.logger.debug(f"Executing Cypher query in get_edges_batch: {query} with params: {{'pairs': {params}}}")
                ...
                async for record in result:
                    if all(k in record for k in ["source", "target", "properties"]):
                        ...
                        if "weight" in props:
                            ...
                            props["weight"] = self.threshold_manager.get_threshold(rel_type_for_threshold)
                        
                        # Add standard timestamp if missing
                        if "extraction_timestamp" not in props:
                            props["extraction_timestamp"] = props.get("timestamp", datetime.now().isoformat())
                            
                        # Ensure description exists
                        if "description" not in props or not props["description"]:
                            props["description"] = f"Relationship of type {props.get('original_type', neo4j_type)} between {source} and {target}"
                ...
                for src, tgt in missing_pairs:
                    ...
                    rel_type = next((p.get("rel_type", "related") for p in params 
                                    if p["src"] == src and p["tgt"] == tgt), "related")
            ...
            return {(pair["src"], pair["tgt"]): {
                ...
                "neo4j_type": self.rel_registry.get_neo4j_type(pair.get("rel_type", "related")),
                "weight": self.threshold_manager.get_threshold(pair.get("rel_type", "related")),
                "confidence": self.threshold_manager.get_threshold(pair.get("rel_type", "related")),
                "description": f"Relationship between {pair['src']} and {pair['tgt']}",
                "keywords": [pair["src"], pair["tgt"], pair.get("rel_type", "related")],
                "extraction_source": "system",
                "extraction_timestamp": current_time,
                "source_id": None,
            } for pair in pairs}
        ...
        try:
            async with self._driver.session(
                ...
                try:
                    ...
                    utils.logger.debug(f"Executing Cypher query in get_node_edges: {query} with params: {{'entity_id': '{source_node_id}'}}")
            ...
            utils.logger.error(f"Error in get_node_edges for {source_node_id}: {str(e)}")
            raise
    ...
    async def get_nodes_edges_batch(
        ...
        async with self._driver.session(
            ...
            """
            utils.logger.debug(f"Executing Cypher query in get_nodes_edges_batch: {query} with params: {{'node_ids': {node_ids}}}")
            result = await session.run(query, node_ids=node_ids)
    ...
    async def upsert_node(self, node_id: str, node_data: dict[str, str]) -> None:
        ...
        if validation_result.has_errors():
            ...
            utils.logger.error(f"Node validation failed for '{node_id}': {error_messages}")
            log_validation_errors(validation_result.errors, f"upsert_node({node_id})")
            raise ValueError(f"Node data validation failed for '{node_id}': {error_messages}")
        
        if validation_result.has_warnings():
            warning_messages = [w.message for w in validation_result.warnings]
            utils.logger.warning(f"Node validation warnings for '{node_id}': {warning_messages}")
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                ...
                async def execute_upsert(tx: AsyncManagedTransaction):
                    ...
                    utils.logger.debug(f"Executing Cypher query in upsert_node: {query} with params: {{'entity_id': '{node_id}', 'properties': {properties}}}")
                    result = await tx.run(
    ...
    async def upsert_edge_detailed(
        ...
        if validation_result.has_errors():
            ...
            log_validation_errors(validation_result.errors, f"upsert_edge({source_id}->{target_id})")
            return False
    ...
    async def enhance_edge_weight_with_embeddings(
        ...
        try:
            ...
            async with await asyncpg.connect(**self.pg_connection_params) as conn:
                ...
                src_embedding = await self._get_entity_embedding(conn, source_id, workspace)
                tgt_embedding = await self._get_entity_embedding(conn, target_id, workspace)
                
                if src_embedding is None or tgt_embedding is None:
                    utils.logger.warning(f"Missing embedding for edge {source_id}->{target_id}")
                    return None
                ...
                await self._update_edge_weight(source_id, target_id, rel_type, new_weight)
    ...
    async def get_edge(
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                utils.logger.debug(f"Executing Cypher query in get_edge (typed): {query} with params: {{'source_id': '{source_id}', 'target_id': '{target_id}'}}")
                result = await session.run(
                    query,
                    source_id=source_id,
                    target_id=target_id
    ...
    async def query_graph(
        ...
        **kwargs
        ...
        try:
            ...
            if query_str.strip().upper().startswith("MATCH") or query_str.strip().upper().startswith("CALL"):
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                ...
                async for record in result:
                    ...
                    result_nodes.append(KnowledgeGraphNode(
    ...
    async def delete_node(self, node_id: str) -> None:
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                ...
                DETACH DELETE n
                """
                utils.logger.debug(f"Executing Cypher query in delete_node: {query} with params: {{'entity_id': '{node_id}'}}")
                result = await session.run(query, entity_id=node_id)
                await result.consume()
    ...
    async def get_knowledge_graph(
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                ...
                utils.logger.debug(f"Executing Cypher query in get_knowledge_graph (nodes): {node_query} with params: {node_params}")
    ...
    async def remove_edges(self, edges: list[tuple[str, str]]) -> None:
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                ...
                edge_params = [{"source": source, "target": target} for source, target in edges]
                
                utils.logger.debug(f"Executing Cypher query in remove_edges: {query} with params: {{'edges': {edge_params}}}")
    ...
    async def remove_nodes(self, node_ids: list[str]) -> None:
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                ...
                utils.logger.debug(f"Executing Cypher query in remove_nodes: {query} with params: {{'node_ids': {node_ids}}}")
                result = await session.run(query, node_ids=node_ids)
                await result.consume()
    ...
    async def run_cypher_query(self, query: str, max_results: int = 50) -> KnowledgeGraph:
        """
    ...
    async def _get_entity_embedding(self, conn, entity_id: str, workspace: str = None) -> Optional[list]:
        """
        ...
        try:
            ...
            utils.logger.warning(f"Error getting entity embedding for {entity_id}: {str(e)}")
            return None

    async def _update_edge_weight(self, source_id: str, target_id: str, rel_type: str, weight: float) -> None:
        """
        ...
        try:
            ...
            async with self._driver.session(database=self._DATABASE) as session:
                utils.logger.debug(f"Executing Cypher query in _update_edge_weight: {query} with params: {{'source_id': '{source_id}', 'target_id': '{target_id}', 'weight': {weight}}}")
                result = await session.run(
                    query,
                    source_id=source_id,
                    target_id=target_id,
