...
def advanced_semantic_chunking(
    ...
    logger.info(f"[advanced_semantic_chunking] Processing content of length: {len(content)}")
...
def get_relationship_types() -> List[str]:
    ...
    try:
        ...
        return [
            "related", "associated", "influences", "uses", "part of", "depends on",
            "creates", "generates", "impacts", "derived from", "requires", "provides",
            "connects", "enables", "facilitates", "improves", "supports", "maintains",
            "implements", "extends", "inherits", "contains", "includes", "consists of",
            "belongs to", "owned by", "managed by", "controlled by", "supervises",
            "reports to", "collaborates", "interacts", "triggers", "causes", "prevents",
            "conflicts", "contradicts", "opposes", "competes", "differs", "resembles",
            "similar to", "equivalent", "represents", "models", "simulates", "emulates",
            "transcends", "encompasses", "categorizes", "characterizes", "defines",
            "describes", "explains", "demonstrates", "validates", "verifies", "confirms",
            "denies", "refutes", "challenges", "questions", "analyzes", "evaluates",
            "measures", "quantifies", "converts", "transforms", "translates", "adapts",
            "modifies", "customizes", "configures", "optimizes", "enhances", "upgrades",
            "replaces", "succeeds", "precedes", "follows", "leads to", "results in",
            "contributes", "participates", "engages", "involves", "attracts",
            "recommends", "suggests"
        ]
...
async def kg_query_with_details(
    ...
    retrieval_details["timings"]["keyword_extraction_ms"] = (time.perf_counter() - t_start_keywords) * 1000
    ...
    if not ll_keywords and query_param.mode in ["local", "hybrid"]:
        logger.warning(f"No low-level keywords, switching from {query_param.mode} to global mode")
        query_param.mode = "global"
    if not hl_keywords and query_param.mode in ["global", "hybrid"]:
        logger.warning(f"No high-level keywords, switching from {query_param.mode} to local mode")
    ...
    retrieval_details["timings"]["context_build_ms"] = (time.perf_counter() - t_start_context) * 1000
    ...
    retrieval_details["timings"]["llm_call_ms"] = (time.perf_counter() - t_start_llm) * 1000
...
async def naive_query_with_details(
    ...
    retrieval_details["timings"]["vector_search_ms"] = (time.perf_counter() - t_start_search) * 1000
    ...
    retrieval_details["retrieved_chunks_summary"] = [
        {
            ...
            "content_summary": (c.get("content", "")[:50] + "...") if c.get("content") else "",
            "file_path": c.get("file_path", "unknown_source")
        }
        for c, r in zip(truncated_chunks, results[:len(truncated_chunks)])
    ]
    
    logger.info(f"Naive query: {len(truncated_chunks)} chunks, top_k: {query_param.top_k}")
...
async def mix_kg_vector_query(
    ...
    async def get_vector_context() -> Tuple[str | None, Dict[str, Any]]:
        ...
        try:
            ...
            for chunk, result in zip(chunks, results):
                if chunk is not None and "content" in chunk:
                    chunk_with_metadata = {
                        ...
                        "file_path": result.get("file_path", chunk.get("file_path", "unknown_source")),
                    }
            ...
            vector_details["retrieved_chunks_after_truncation_count"] = len(truncated_chunks)
    ...
    (kg_context_str, kg_ret_details), (vector_context_str, vec_ret_details) = await asyncio.gather(
        get_kg_context(), get_vector_context()
    )
    combined_retrieval_details["timings"]["hybrid_retrieval_ms"] = (time.perf_counter() - t_start_hybrid) * 1000
    ...
    if kg_ret_details and isinstance(kg_ret_details, dict):
        combined_retrieval_details["effective_hl_keywords"] = kg_ret_details.get("effective_hl_keywords", [])
        combined_retrieval_details["effective_ll_keywords"] = kg_ret_details.get("effective_ll_keywords", [])
...
""".strip()
    ...
    # Build hybrid prompt
    sys_prompt = (system_prompt if system_prompt else PROMPTS.get("mix_rag_response", PROMPTS["rag_response"])).format(
        kg_context=kg_context_str if kg_context_str else "No relevant knowledge graph information found",
        vector_context=vector_context_str if vector_context_str else "No relevant text information found",
        response_type=query_param.response_type,
        history=history_context,
    )
    ...
    combined_retrieval_details["timings"]["llm_call_ms"] = (time.perf_counter() - t_start_llm) * 1000
...
async def _build_query_context_with_details(
    ...
    if query_param.mode == "local":
        entities_context, relations_context, text_units_context, details = await _get_node_data_with_details(
            ll_keywords,
            knowledge_graph_inst,
            entities_vdb,
            text_chunks_db,
        ...
        entities_context, relations_context, text_units_context, details = await _get_edge_data_with_details(
            hl_keywords,
            knowledge_graph_inst,
            relationships_vdb,
            text_chunks_db,
        ...
        retrieval_details["effective_hl_keywords"] = hl_keywords.split(", ") if hl_keywords else []
        retrieval_details["effective_ll_keywords"] = ll_keywords.split(", ") if ll_keywords else []
...
async def _get_node_data_with_details(
    ...
    if isinstance(entities_context, list):
        retrieval_details["retrieved_entities_after_truncation_count"] = len(entities_context)
        retrieval_details["retrieved_entities_summary"] = [
            {
                "name": e.get("entity", ""),
                "type": e.get("type", "UNKNOWN"),
                "rank": e.get("rank", 0),
                "description_summary": (e.get("description", "")[:50] + "...") if e.get("description") else ""
            }
        ...
        retrieval_details["retrieved_relationships_summary"] = [
            {
                ...
                "description_summary": (r.get("description", "")[:50] + "...") if r.get("description") else ""
            }
...
async def _get_edge_data_with_details(
    ...
    if isinstance(relations_context, list):
        retrieval_details["retrieved_relationships_after_truncation_count"] = len(relations_context)
        retrieval_details["retrieved_relationships_summary"] = [
            {
                "source": r.get("entity1", ""),
                "target": r.get("entity2", ""),
